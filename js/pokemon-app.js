...
let gameState = {
	...
	messages: "",
	time: 5000,
	gameMessage: "Catch all the locations before the time is up!"
}
...
let map = new InteractiveMap({
	...
	update() {
		...
		if (gameState.time > 0){
			...
			gameState.gameMessage = "Times up! How many locations did you catch?"
		}
	...
	onEnterRange: (landmark, newLevel, oldLevel, dist) => {
		...
		if ((newLevel == 2) && (gameState.time >0)) {
			...
			if (!gameState.captured.includes(landmark.name)) {
				gameState.captured.push(landmark.name + ": " +landmark.points)
				// Add a message
				gameState.messages= `You have visited ${landmark.name}! You gained ${landmark.points} points!`
			}
			gameState.amount = gameState.captured.length
		...
		<header></header>
			<div id="main-columns">
				...
				<div class="main-column" style="flex:1;overflow:scroll;max-height:200px">
					<h2> NU Catch!!! </h2> <br>
					<h3> {{gameState.gameMessage}} </h3> <br>
					Time Left: {{gameState.time}} <br>
					Total Caught: {{gameState.amount}}<br>
					Total points: {{gameState.points}}<br>
					Caputured Locations: {{gameState.captured}}<br>
